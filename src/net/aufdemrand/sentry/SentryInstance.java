...
public class SentryInstance {
                        ...
                        if (faction != null) {
                            ...
                            if (this.hasTargetType(factionenemies)) {
                                for (String s : FactionEnemies) {
                                    if (FactionsUtil.isFactionEnemy(myNPC.getEntity().getWorld().getName(), s, faction))
                                        return true;
                                }
                            }
                        }
    ...
    public void die(boolean runscripts, org.bukkit.event.entity.EntityDamageEvent.DamageCause cause) {
        ...
        if (this.DropInventory)
            myNPC.getEntity().getLocation().getWorld().spawn(myNPC.getEntity().getLocation(), ExperienceOrb.class).setExperience(plugin.SentryEXP);
    ...
    public void Fire(LivingEntity theEntity) {
        ...
        if (lightning) {
            ...
            if (myProjectile == org.bukkit.entity.ThrownPotion.class) {
                net.minecraft.server.v1_7_R1.World nmsWorld = ((CraftWorld) myNPC.getEntity().getWorld()).getHandle();
        ...
        if (finaldamage > 0) {
            ...
            if (getHealth() - finaldamage <= 0) {
                ...
                if (event.getDamager() instanceof HumanEntity)
                    ((CraftLivingEntity) myNPC.getEntity()).getHandle().killer = (EntityHuman) ((CraftLivingEntity) event.getDamager()).getHandle();
